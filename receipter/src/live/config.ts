...
import type { SafeConfig, SealEncryptionMode, ReceipterConfig, ReceipterMode, WalrusUploadStrategy } from './types.js';
...
const DEFAULT_MEMWAL_NAMESPACE = 'receipter';

export function loadReceipterConfig(env: NodeJS.ProcessEnv = process.env): ReceipterConfig {
  const mode = parseMode(env.RECEIPTER_MODE ?? 'sui-dev');
  const port = parsePort(env.RECEIPTER_PORT);
  const maxPaymentSui = parseAmount(env.RECEIPTER_MAX_PAYMENT_SUI ?? DEFAULT_MAX_PAYMENT_SUI);
  const receiptsDir = path.resolve(env.RECEIPTER_RECEIPTS_DIR ?? DEFAULT_RECEIPTS_DIR);
  const workerAgentId = blankToUndefined(env.RECEIPTER_WORKER_AGENT_ID) ?? DEFAULT_WORKER_AGENT_ID;
  ...
  const workerAgentAddress = blankToUndefined(env.RECEIPTER_WORKER_AGENT_ADDRESS) ?? suiOperatorAddress;
  const workerAgentPassportObjectId = blankToUndefined(env.RECEIPTER_WORKER_AGENT_PASSPORT_OBJECT_ID);
  ...
  if (mode === 'sui' && memoryBackend === 'memwal' && missingMemwalSettings.length > 0) {
    throw new Error(
      `RECEIPTER_MODE=sui with MEMORY_BACKEND=memwal requires ${missingMemwalSettings.join(', ')}. ` +
...
function parseMode(value: string): ReceipterMode {
  if (value === 'sui-dev' || value === 'sui') {
    return value;
  }

  throw new Error(`Invalid RECEIPTER_MODE: ${value}. Expected sui-dev or sui.`);
}
...
function parsePort(value: string | undefined): number {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new Error(`Invalid RECEIPTER_PORT: ${value}.`);
  }
  return port;
}
