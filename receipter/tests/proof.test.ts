...
describe('renderReceiptProof', () => {
  it('renders a judge-readable Sui proof without private notes', () => {
    ...
    expect(proof).toContain('# Receipter Memory Proof: run_proof');
  ...
  it('stores evidence through the Walrus HTTP publisher response format', async () => {
    ...
    expect(String(uploads[0]!.init?.body)).toContain('"schema": "receipter.sui.evidence.v1"');
  ...
  it('renders the MemWal reputation fact as a searchable work-memory summary', () => {
    ...
    expect(fact).toContain('Receipter verified work memory for agent sui_worker.');
  ...
  it('refuses buyer-private production uploads without a live Seal SDK integration', async () => {
    ...
    ).rejects.toThrow('Seal encryption is required for buyer_private memory in RECEIPTER_MODE=sui');
  });
});

function testWalrusConfig(overrides: Partial<ReceipterConfig> = {}): ReceipterConfig {
  return {
    mode: 'sui',
    suiNetwork: 'testnet',
    suiRpcUrl: 'https://fullnode.testnet.sui.io:443',
