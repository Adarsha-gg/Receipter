...
export function renderReceiptProof(receipt: LiveRunReceipt): string {
  const lines = [
    ...
    `- Reputation memory records: ${receipt.reputationSnapshot?.memoryCount ?? 0}`,
    `- Reputation average claim support: ${receipt.reputationSnapshot?.averageClaimSupport ?? 'none'}`,
    ...
    `- Reputation last memory: ${receipt.reputationSnapshot?.lastMemoryId ?? 'none'}`,
...
function renderWalrusMemoryRecord(receipt: LiveRunReceipt): string[] {
  ...
  return [
    ...
    `- Payment bound: ${memory.marketplaceProof.paymentBound ? 'yes' : 'no'}`,
    `- Worker selected: ${memory.marketplaceProof.workerSelected ? 'yes' : 'no'}`,
    `- Source verified: ${memory.marketplaceProof.sourceVerified ? 'yes' : 'no'}`,
    `- Walrus stored: ${memory.marketplaceProof.walrusStored ? 'yes' : 'no'}`,
    `- Sui anchored: ${memory.marketplaceProof.suiAnchored ? 'yes' : 'no'}`,
    `- Work order: ${memory.workOrderId ?? 'not recorded'}`,
    `- Payment intent: ${memory.paymentIntentId ?? 'not recorded'}`,
    `- Selected bid: ${memory.selectedBidId ?? 'not recorded'}`,
    `- Amount: ${memory.amountSui ?? 'not recorded'} SUI (${memory.amountMist ?? 'not recorded'} MIST)`,
...
function renderWorkerReputation(receipt: LiveRunReceipt): string[] {
  ...
  return [
    ...
    `- Memory records: ${snapshot.memoryCount}`,
    `- Average claim support: ${snapshot.averageClaimSupport ?? 'none'}`,
    ...
    `- Last memory: ${snapshot.lastMemoryId ?? 'none'}`,
