...
describe('live memory smoke helpers', () => {
  ...
  it('verifies Walrus readback contains the smoke run and evidence schema', async () => {
    const readback = await verifyWalrusSmokeReadback(
      ...
      'memwal_smoke_1',
      async () =>
        new Response('{"schema": "tenderboard.sui.evidence.v1", "run": { "runId": "memwal_smoke_1" }}', {
          status: 200,
        }),
    );
  ...
  it('fails Walrus readback when the blob does not contain the expected run', async () => {
    const readback = await verifyWalrusSmokeReadback(
      ...
      async () =>
        new Response('{"schema": "tenderboard.sui.evidence.v1", "run": { "runId": "other" }}', {
          status: 200,
        }),
    );
...
function testConfig(): TenderBoardConfig {
  return {
    mode: 'sui',
    port: 0,
    maxPaymentSui: '0.050',
